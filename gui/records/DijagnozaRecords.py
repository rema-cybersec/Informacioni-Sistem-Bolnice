...
from utils.Utils import update_dijagnoza_record, delete_dijagnoza_record
import base64
...
class DijagnozaRecords(Records):
    ...
    def initialize_left_frame(self):
        ...
        self.sifra = ctk.CTkLabel(master=self.left_frame, text="sifra:")
        self.sifra.grid(row=0, column=0, sticky="nsew", padx=(20, 20), pady=(20, 20))

        self.naziv = ctk.CTkLabel(master=self.left_frame, text="naziv:")
        self.naziv.grid(row=1, column=0, sticky="nsew", padx=(20, 20), pady=(20, 20))

        self.opis = ctk.CTkLabel(master=self.left_frame, text="opis:")
        self.opis.grid(row=2, column=0, sticky="nsew", padx=(20, 20), pady=(20, 20))
    ...
    def initialize_right_frame(self):
        ...
        self.sifra_field = ctk.CTkEntry(master=self.right_frame, placeholder_text=self.dijagnoza["sifra"])
        self.sifra_field.grid(row=0, column=0, sticky="nswe", padx=(20, 20), pady=(20, 20))

        self.naziv_field = ctk.CTkEntry(master=self.right_frame, placeholder_text=base64.b64decode(self.dijagnoza["naziv"].encode("utf-8").decode("utf-8")))
        self.naziv_field.grid(row=1, column=0, sticky="nswe", padx=(20, 20), pady=(20, 20))

        self.opis_field = ctk.CTkEntry(master=self.right_frame, placeholder_text=base64.b64decode(self.dijagnoza["opis"].encode("utf-8").decode("utf-8")))
        self.opis_field.grid(row=2, column=0, sticky="nswe", padx=(20, 20), pady=(20, 20))
    ...
    def allow_action(self):
        if self.key_obj.isValid:
            if self.action == "update":
                update_dijagnoza_record(self)
            elif self.action == "delete":
                delete_dijagnoza_record(self)
        self.action="other"
